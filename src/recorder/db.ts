...
export const initializeDatabase = async () => {
    return new Promise<IDBDatabase>((resolve, reject) => {
        ...
        idbOpenDBRequest.addEventListener("upgradeneeded", () => {
            const recordingsObject = idbOpenDBRequest.result.createObjectStore("recordings", { keyPath: "key" });
...
export const startTransaction = (db: IDBDatabase, storeName: string, mode: IDBTransactionMode, options?: IDBTransactionOptions) => {
    type T = void;
    ...
    transaction.addEventListener("error", (event) => transactionDone.reject(transaction.error));
    transaction.addEventListener("abort", (event) => transactionDone.reject(transaction.error));
    return {
        transaction,
        promise: transactionDone.promise,
        then: <TResult1 = T, TResult2 = never>(onfulfilled?: ((value: T) => TResult1 | PromiseLike<TResult1>) | undefined | null, onrejected?: ((reason: any) => TResult2 | PromiseLike<TResult2>) | undefined | null) => transactionDone.promise.then(onfulfilled, onrejected),
    };
}

export const insertRecordsIntoStore = <A>(transaction: IDBTransaction, storeName: string, ...values: A[]) => {
...
export const recording = {
    create: async (...records: RecordDTO[]) => {
        const { transaction, promise: transactionPromise } = startTransaction(db, "recordings", "readwrite", { durability: "strict" });
    ...
    list: async function* (): AsyncGenerator<RecordDTO> {
        const { transaction, promise: transactionPromise } = startTransaction(db, "recordings", "readonly", { durability: "strict" });
