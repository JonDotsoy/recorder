...
class BytesFormat {
    ...
    format(bytes: number): string {
        ...
        if (bytes < 1024 * 1024 * 1024 * 1024 * 1024) {
            return this.#terabyteFormatter.format(bytes / (1024 * 1024 * 1024 * 1024));
        }

        return this.#petabyteFormatter.format(bytes / (1024 * 1024 * 1024 * 1024 * 1024));
    }
}
            ...
            {records?.sort((a, b) => b.timestamp - a.timestamp)?.map((record) => (
                <div className="grid grid-cols-[1fr_auto] gap-4 border-b border-gray-200" key={record.key}>
                    <div key={record.key} className="flex flex-col gap-2 p-4">
                        ...
                        <p className="text-gray-500">{new Date(record.timestamp).toLocaleString(undefined, { timeStyle: 'full', dateStyle: 'full' })}</p>
                        <p className="text-gray-500">{new BytesFormat(undefined, { unitDisplay: 'long' }).format(record.data.size)}</p>
                    </div>
